...
frontend:
  - task: "Service page hero background from uploaded media (HeroMedia)"
    ...
    status_history:
      ...
      - working: "NA"
        agent: "main"
        comment: "ROLLED OUT to all remaining service pages using each page's own banner slot: events (events-banner), drone-services (drone-services-banner), portraits-headshots (portraits-headshots-banner), editorial-portfolio (editorial-portfolio-banner), live-streaming (live-streaming-banner), and the generic /services/[slug] page (`${slug}-banner`, covers fashion-shoots, boudoir-shoots, product-ecommerce, brand-content, corporate-industrial, real-estate-architectural, influencer-celebrity, podcast-production, editing-retouching, album-design, design-services, family-kids, etc.). In each client the hardcoded hero background (video/Image) was replaced with <HeroMedia slot=... fallbackImage=... fallbackVideo=... /> and the separate <PageBanner> above the page was removed. These banner slots currently have NO uploads, so they show the page's original fallback hero. All pages return HTTP 200 and lint clean. Needs retest: confirm each service page's FIRST section is its hero (no separate banner above), fallback hero renders with text overlay, and photo-then-video works when media is present."
