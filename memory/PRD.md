...
- **2026-06 Site-wide mobile overflow audit**: applied the boudoir fix across all service pages. `grid grid-cols-12` (gap ≥32px) → `flex flex-col ... md:grid`/`lg:grid` (matching child breakpoints) in weddings/page.js (6 sections), [slug]/ServicePageClient.jsx (4 sections), booking/page.js, client/ClientAuth.jsx. Shared Footer (Chrome.jsx) → `gap-x-6 gap-y-10 md:gap-10` (kept 2-up mobile columns). Verified by testing_agent at 360/390 across 11 pages: no horizontal scroll, no cropped content (iteration_6).
