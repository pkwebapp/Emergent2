      ...
      <section className="py-20 md:py-24 bg-[#161514] text-white" data-testid="events-pricing-section">
        <div className="container mx-auto max-w-[1200px] px-6 md:px-10">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {pricing.map((plan) => (
              <FadeIn key={plan.name} className={`rounded-[1.75rem] p-6 border flex flex-col ${plan.featured ? 'bg-[#FF5B22] border-[#FF5B22] text-white' : 'bg-white/5 border-white/10'}`} data-testid={`events-pricing-${plan.name.toLowerCase().replaceAll(' ', '-')}`}>
                ...
                <Link href={`/booking?service=events&package=${encodeURIComponent(plan.name)}&price=${encodeURIComponent(plan.price)}`} data-testid={`events-pricing-book-${plan.name.toLowerCase().replaceAll(' ', '-')}`} className="inline-flex justify-center items-center gap-2 bg-white text-[#161514] px-5 py-3 rounded-full text-sm font-semibold hover:bg-[#161514] hover:text-white transition-colors">Book this package <ArrowRight size={14} /></Link>
                  <a href={waLink({ service: 'Event Photography', page: 'Events', pkg: plan.name, price: plan.price })} target="_blank" rel="noreferrer" data-testid={`events-pricing-whatsapp-${plan.name.toLowerCase().replaceAll(' ', '-')}`} className="inline-flex justify-center items-center gap-2 border border-white/40 text-white px-5 py-3 rounded-full text-sm font-semibold hover:bg-white/10 transition-colors"><MessageCircle size={14} /> Enquire on WhatsApp</a>
                </div>
              </FadeIn>
            ))}
          </div>
        ...
        <div className="container mx-auto max-w-[1200px] px-6 md:px-10">
          <div className="rounded-[2rem] bg-[#FF5B22] text-white p-8 md:p-12 grid md:grid-cols-[1.15fr_0.85fr] gap-8 items-center overflow-hidden relative">
            ...
            <FadeIn className="relative flex flex-col sm:flex-row md:flex-col gap-3 md:items-start">
              ...
              <a href={waLink({ service: 'Event Photography', page: 'Events' })} target="_blank" rel="noreferrer" data-testid="events-final-whatsapp-link" className="inline-flex justify-center items-center gap-3 border border-white/35 text-white px-7 py-3.5 rounded-full text-sm font-semibold hover:bg-white hover:text-[#161514] transition-colors"><MessageCircle size={15} /> WhatsApp the team</a>
            </FadeIn>
          </div>
        </div>
      </section>
